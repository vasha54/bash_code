...
make_addons_product_data_import() {
    local name_base="$1"
    local directory_base="desoft_mtz_products_data_import_${name_base}"
    ...
    if [ -f "$directory_base/data/$name_base.xml" ]; then
        ...
        local file_data=$directory_base/data/$name_base.xml
        touch $file_data
        echo "<?xml version=\"1.0\" encoding=\"utf-8\"?>" >> $file_data
        echo "<odoo noupdate=\"1\">" >> $file_data
        echo "   <data>" >> $file_data
        echo "   </data>" >> $file_data
        echo "</odoo>" >> $file_data
        echo "Fichero creado en: $directory_base/data/$name_base.xml"
    fi
}
...
make_manifest_addons(){
    ...
    echo { >> $file
    echo "  'name': 'Desoft MTZ - Products Data Import - ,  - $1'," >> $file
    echo "  'summary': ''," >> $file
    ...
    echo "  'depends': [" >> $file
    echo "      'base'," >> $file
    echo "      'desoft_mtz_products'," >> $file
    echo "      'desoft_mtz_products_data_import'," >> $file
    echo "  ]," >> $file
    echo "  'data': [" >> $file
...
while IFS= read -r linea; do
    ...
    make_addons_product_data_import "$linea"
